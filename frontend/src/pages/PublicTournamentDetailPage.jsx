...
export default function PublicTournamentDetailPage() {
  ...
  const load = useCallback(async () => {
    try {
      const response = await tournamentApi.getPublic(slug);
      setTournament(unwrapData(response).tournament);
      setError('');
    } catch (requestError) {
  ...
  return (
    <>
      ...
      <section className="mt-8 rounded-3xl border border-white/[0.08] bg-white/[0.035] p-6">
        ...
        <p className="mt-2 text-white/50">Only confirmed participants are public. Groups, fixtures, standings, statistics, and awards are intentionally not shown yet.</p>
