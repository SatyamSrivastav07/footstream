...
export default function TeamBrandingUploader({ kind, initialImage, uploadUrl, deleteUrl }) {
  ...
  const upload = async () => {
    ...
    try {
      ...
      setImageUrl(response.data.data.image.imageUrl || "");
      setPendingFile(null);
      setPreview("");
  ...
  return (
    <article className="rounded-2xl border border-white/[0.08] bg-black/10 p-4">
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        ...
        <button type="button" className="secondary-button" onClick={() => inputRef.current?.click()} disabled={busy}>Choose</button>
        <button type="button" className="primary-button" onClick={upload} disabled={busy || !pendingFile}><Upload size={16} /> {busy ? "Saving..." : "Upload"}</button>
        {imageUrl && <button type="button" className="secondary-button" onClick={remove} disabled={busy}><Trash2 size={16} /> Remove</button>}
      </div>
    </article>
  );
}
