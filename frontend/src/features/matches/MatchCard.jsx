  ...
  return (
    <article className="rounded-3xl border border-white/[0.08] bg-white/[0.025] p-5 transition hover:border-white/[0.14]">
      <div className="flex items-start justify-between gap-4">
        <div><span className={`status-badge ${statusClass[match.status]}`}>{label(match.status)}</span><p className="mt-3 text-xs font-bold uppercase tracking-[0.16em] text-emerald-100/35">{label(match.matchType)} · {label(match.teamSide)}</p></div>
