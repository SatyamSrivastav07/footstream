...
import { emptyMatch, FORMAT_FORMATIONS, FORMAT_STARTERS, label, MATCH_TYPES, TEAM_SIDES, toLocalInput } from './constants.js';
...
export default function MatchForm({ initialMatch, teamName, saving, serverError, serverFieldErrors = {}, onSubmit }) {
  ...
  const validate = () => {
    ...
    if (canEditDetails && form.temporaryPlayers.some((player) => player.name.trim().length < 2)) next.temporaryPlayers = 'Every temporary opponent player needs a name.';
    setErrors(next);
    ...
    <form onSubmit={submit} className="space-y-7" noValidate>
      ...
      <section className="panel"><SectionHeader number="01" title="Match details" copy="Schedule the fixture and record the opponent without creating another team." />
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          <Field label="Opponent name" error={fieldError('opponent')}><input className="field-input mt-2" value={form.opponentName} disabled={!canEditDetails} onChange={(e) => update('opponentName', e.target.value)} /></Field>
          <Field label="Tournament"><input className="field-input mt-2" value={form.tournament} disabled={!canEditDetails} onChange={(e) => update('tournament', e.target.value)} /></Field>
          <Field label="Venue" error={fieldError('venue')}><input className="field-input mt-2" value={form.venue} disabled={!canEditDetails} onChange={(e) => update('venue', e.target.value)} /></Field>
          <Field label="Match type"><select className="field-input mt-2" value={form.matchType} disabled={!canEditDetails} onChange={(e) => update('matchType', e.target.value)}>{MATCH_TYPES.map((type) => <option key={type} value={type}>{label(type)}</option>)}</select></Field>
          {initialMatch?.sourceChallenge && <Field label="Match Format"><input className="field-input mt-2" value={matchFormat} readOnly aria-readonly="true" /></Field>}
          <Field label="Team side"><select className="field-input mt-2" value={form.teamSide} disabled={!canEditDetails} onChange={(e) => update('teamSide', e.target.value)}>{TEAM_SIDES.map((side) => <option key={side} value={side}>{label(side)}</option>)}</select></Field>
          <Field label="Kickoff" error={fieldError('scheduledAt')}><input className="field-input mt-2" type="datetime-local" value={form.scheduledAt} disabled={!canEditDetails} onChange={(e) => update('scheduledAt', e.target.value)} /></Field>
          <Field label="Formation"><select className="field-input mt-2" value={form.formation} onChange={(e) => update('formation', e.target.value)}><option value="">Not specified</option>{compatibleFormations.map((formation) => <option key={formation} value={formation}>{label(formation)}</option>)}</select></Field>
          ...
          <Field label="Notes" className="sm:col-span-2 xl:col-span-3"><textarea className="field-input mt-2 min-h-24 resize-y" maxLength="2000" value={form.notes} disabled={!canEditDetails} onChange={(e) => update('notes', e.target.value)} /></Field>
        </div>
        {canEditDetails && <div className="mt-6 border-t border-white/[0.07] pt-5"><div className="flex items-center justify-between gap-3"><div><h3 className="font-semibold text-white">Temporary opponent players</h3><p className="mt-1 text-xs text-emerald-100/40">Optional names stored only on this match.</p></div><button type="button" className="secondary-button px-3" onClick={addTemporaryPlayer}><Plus size={15} /> Add name</button></div>
      ...
      <section className="panel"><SectionHeader number="02" title="Starting lineup" copy={`Only active and available permanent squad members can be selected. ${matchFormat} requires ${requiredStarters} starters.`} count={`${form.startingPlayerIds.length}/${requiredStarters}`} />
      ...
      <section className="rounded-3xl border border-lime-300/15 bg-lime-300/[0.055] p-6"><SectionHeader number="04" title="Review & save" copy="Confirm the fixture and selected match-day squad." />
        <dl className="mt-6 grid gap-4 text-sm sm:grid-cols-2 lg:grid-cols-4"><Summary label="Fixture" value={`${teamName} vs ${form.opponentName || 'Opponent'}`} /><Summary label="Kickoff" value={form.scheduledAt ? new Date(form.scheduledAt).toLocaleString() : 'Not set'} /><Summary label="Formation" value={form.formation === 'custom' ? form.customFormation || 'Custom' : form.formation || 'Not set'} /><Summary label="Squad" value={`${matchFormat}: ${form.startingPlayerIds.length} starters - ${form.substitutePlayerIds.length} substitutes`} /></dl>
