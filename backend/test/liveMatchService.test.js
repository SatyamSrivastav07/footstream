...
test('assisted goal stores saved lineup snapshots and scorer differs from assist', () => {
  ...
  assert.throws(() => buildEventData({ match: match(), events: [], type: 'goal', input: { scoringSide: 'team', playerId: playerId(2), assistPlayerId: playerId(2) } }), (error) => error.code === 'ASSIST_SAME_AS_SCORER');
});
...
test('saved lineup snapshot remains eligible despite later player changes and non-lineup player is rejected', () => {
  ...
  assert.throws(() => buildEventData({ match: match(), events: [], type: 'goal', input: { scoringSide: 'team', playerId: playerId(40) } }), (error) => error.code === 'PLAYER_NOT_IN_LINEUP');
});
...
test('invalid substitutions reject non-field, non-bench, repeat entry, and re-entry', () => {
  const value = match();
  assert.throws(() => buildEventData({ match: value, events: [], type: 'substitution', input: { playerOutId: playerId(12), playerInId: playerId(13) } }), (error) => error.code === 'PLAYER_OUT_NOT_ON_FIELD');
  assert.throws(() => buildEventData({ match: value, events: [], type: 'substitution', input: { playerOutId: playerId(5), playerInId: playerId(6) } }), (error) => error.code === 'PLAYER_IN_NOT_ON_BENCH');
  ...
  assert.throws(() => buildEventData({ match: value, events: [first], type: 'substitution', input: { playerOutId: playerId(6), playerInId: playerId(12) } }), (error) => ['PLAYER_IN_NOT_ON_BENCH', 'PLAYER_ALREADY_ENTERED'].includes(error.code));
  assert.throws(() => buildEventData({ match: value, events: [first], type: 'substitution', input: { playerOutId: playerId(7), playerInId: playerId(5) } }), (error) => error.code === 'PLAYER_CANNOT_REENTER');
});
